...
const formats = [
  {
    ...
    note: "Your provider recommends the best format for you.",
    img: formatInjection,
...
const WeightLoss = () => {
  ...
  return (
    <PageLayout title="GLP-1 Weight Loss">
        ...
        <div className="max-w-[1280px] mx-auto grid lg:grid-cols-2 gap-10 items-center fade-up">
          <div>
            ...
            <div className="flex flex-wrap gap-4">
              {["From $179/mo — All-Inclusive", "Money-Back Guarantee", "No Membership Fee", "Free Discreet Shipping", "HSA/FSA Accepted"].map((t) => (
      ...
      {/* Medication options — From prices only */}
      ...
      <div className="bg-background px-5 md:px-12 pt-14 pb-16 border-b border-warm-100">
        <div className="max-w-[1280px] mx-auto fade-up">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {formats.map((fmt) => (
              <div key={fmt.title} className="bg-card border border-warm-100 rounded-2xl overflow-hidden flex flex-col">
                ...
                <div className="p-7 flex flex-col flex-1">
                  <span className={`inline-block self-start text-[0.62rem] font-bold tracking-[0.08em] uppercase px-2.5 py-1 rounded mb-4 ${fmt.badgeRed ? "bg-red text-white" : "bg-warm-100 text-warm-600"}`}>
                    {fmt.badge}
                  </span>
                  ...
                  <p className="text-[0.72rem] text-warm-400 italic mt-auto">{fmt.note}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-background px-5 md:px-12 py-10 border-b border-warm-100">
        <div className="max-w-[1280px] mx-auto fade-up">
          <div className="bg-success/10 border border-success/30 rounded-2xl px-7 py-7 flex flex-col sm:flex-row items-start sm:items-center gap-5">
            ...
            <div className="flex-1">
              ...
              <p className="text-[0.83rem] text-warm-600 leading-[1.7]">
                Pay for your GLP-1 treatment with pre-tax health savings dollars. Patients in the 30% tax bracket effectively pay{" "}
                <strong className="text-warm-800">~$125/mo</strong> instead of $179/mo when using their HSA or FSA card. That's up to 40% in real savings on top of an already all-inclusive price.
              </p>
            </div>
            <a href="/health-check" className="bg-red hover:bg-red-dark text-primary-foreground font-bold px-6 py-3 rounded-lg text-[0.85rem] transition-colors whitespace-nowrap">
              See If I Qualify →
            </a>
          </div>
        </div>
